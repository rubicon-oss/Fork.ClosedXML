...
namespace ClosedXML_Tests.Excel.CalcEngine
{
    ...
    public class StatisticalTests
    {
        ...
        {
            double value;
            value = workbook.Evaluate(true, "AVERAGE(-27.5,93.93,64.51,-70.56)").CastTo<double>();
            Assert.AreEqual(15.095, value, tolerance);

            var ws = workbook.Worksheets.First();
            value = ws.Evaluate(true, "AVERAGE(G3:G45)").CastTo<double>();
            Assert.AreEqual(49.3255814, value, tolerance);

            Assert.That(() => ws.Evaluate(true, "AVERAGE(D3:D45)"), Throws.TypeOf<ApplicationException>());
        }
        ...
        public void Count()
        {
            var ws = workbook.Worksheets.First();
            int value;
            value = ws.Evaluate(true, @"=COUNT(D3:D45)").CastTo<int>();
            Assert.AreEqual(0, value);

            value = ws.Evaluate(true, @"=COUNT(G3:G45)").CastTo<int>();
            Assert.AreEqual(43, value);

            value = ws.Evaluate(true, @"=COUNT(G:G)").CastTo<int>();
            Assert.AreEqual(43, value);

            value = workbook.Evaluate(true, @"=COUNT(Data!G:G)").CastTo<int>();
            Assert.AreEqual(43, value);
        }
        ...
        public void CountA()
        {
            var ws = workbook.Worksheets.First();
            int value;
            value = ws.Evaluate(true, @"=COUNTA(D3:D45)").CastTo<int>();
            Assert.AreEqual(43, value);

            value = ws.Evaluate(true, @"=COUNTA(G3:G45)").CastTo<int>();
            Assert.AreEqual(43, value);

            value = ws.Evaluate(true, @"=COUNTA(G:G)").CastTo<int>();
            Assert.AreEqual(44, value);

            value = workbook.Evaluate(true, @"=COUNTA(Data!G:G)").CastTo<int>();
            Assert.AreEqual(44, value);
        }
        ...
        public void CountBlank()
        {
            var ws = workbook.Worksheets.First();
            int value;
            value = ws.Evaluate(true, @"=COUNTBLANK(B:B)").CastTo<int>();
            Assert.AreEqual(1048532, value);

            value = ws.Evaluate(true, @"=COUNTBLANK(D43:D49)").CastTo<int>();
            Assert.AreEqual(4, value);

            value = workbook.Evaluate(true, @"=COUNTBLANK(E3:E45)").CastTo<int>();
            Assert.AreEqual(0, value);
        }
        ...
        public void CountIf()
        {
            var ws = workbook.Worksheets.First();
            int value;
            value = ws.Evaluate(true, @"=COUNTIF(D3:D45,""Central"")").CastTo<int>();
            Assert.AreEqual(24, value);

            value = ws.Evaluate(true, @"=COUNTIF(D:D,""Central"")").CastTo<int>();
            Assert.AreEqual(24, value);

            value = workbook.Evaluate(true, @"=COUNTIF(Data!D:D,""Central"")").CastTo<int>();
            Assert.AreEqual(24, value);
        }
        ...
        public void Max()
        {
            var ws = workbook.Worksheets.First();
            int value;
            value = ws.Evaluate(true, @"=MAX(D3:D45)").CastTo<int>();
            Assert.AreEqual(0, value);

            value = ws.Evaluate(true, @"=MAX(G3:G45)").CastTo<int>();
            Assert.AreEqual(96, value);

            value = ws.Evaluate(true, @"=MAX(G:G)").CastTo<int>();
            Assert.AreEqual(96, value);

            value = workbook.Evaluate(true, @"=MAX(Data!G:G)").CastTo<int>();
            Assert.AreEqual(96, value);
        }
        ...
        public void Min()
        {
            var ws = workbook.Worksheets.First();
            int value;
            value = ws.Evaluate(true, @"=MIN(D3:D45)").CastTo<int>();
            Assert.AreEqual(0, value);

            value = ws.Evaluate(true, @"=MIN(G3:G45)").CastTo<int>();
            Assert.AreEqual(2, value);

            value = ws.Evaluate(true, @"=MIN(G:G)").CastTo<int>();
            Assert.AreEqual(2, value);

            value = workbook.Evaluate(true, @"=MIN(Data!G:G)").CastTo<int>();
            Assert.AreEqual(2, value);
        }
        ...
        public void StDev()
        {
            var ws = workbook.Worksheets.First();
            double value;
            Assert.That(() => ws.Evaluate(true, @"=STDEV(D3:D45)"), Throws.TypeOf<ApplicationException>());

            value = ws.Evaluate(true, @"=STDEV(H3:H45)").CastTo<double>();
            Assert.AreEqual(47.34511769, value, tolerance);

            value = ws.Evaluate(true, @"=STDEV(H:H)").CastTo<double>();
            Assert.AreEqual(47.34511769, value, tolerance);

            value = workbook.Evaluate(true, @"=STDEV(Data!H:H)").CastTo<double>();
            Assert.AreEqual(47.34511769, value, tolerance);
        }
        ...
        public void StDevP()
        {
            var ws = workbook.Worksheets.First();
            double value;
            Assert.That(() => ws.Evaluate(true, @"=STDEVP(D3:D45)"), Throws.InvalidOperationException);

            value = ws.Evaluate(true, @"=STDEVP(H3:H45)").CastTo<double>();
            Assert.AreEqual(46.79135458, value, tolerance);

            value = ws.Evaluate(true, @"=STDEVP(H:H)").CastTo<double>();
            Assert.AreEqual(46.79135458, value, tolerance);

            value = workbook.Evaluate(true, @"=STDEVP(Data!H:H)").CastTo<double>();
            Assert.AreEqual(46.79135458, value, tolerance);
        }
        ...
        public void Var()
        {
            var ws = workbook.Worksheets.First();
            double value;
            Assert.That(() => ws.Evaluate(true, @"=VAR(D3:D45)"), Throws.InvalidOperationException);

            value = ws.Evaluate(true, @"=VAR(H3:H45)").CastTo<double>();
            Assert.AreEqual(2241.560169, value, tolerance);

            value = ws.Evaluate(true, @"=VAR(H:H)").CastTo<double>();
            Assert.AreEqual(2241.560169, value, tolerance);

            value = workbook.Evaluate(true, @"=VAR(Data!H:H)").CastTo<double>();
            Assert.AreEqual(2241.560169, value, tolerance);
        }
        ...
        public void VarP()
        {
            var ws = workbook.Worksheets.First();
            double value;
            Assert.That(() => ws.Evaluate(true, @"=VARP(D3:D45)"), Throws.InvalidOperationException);

            value = ws.Evaluate(true, @"=VARP(H3:H45)").CastTo<double>();
            Assert.AreEqual(2189.430863, value, tolerance);

            value = ws.Evaluate(true, @"=VARP(H:H)").CastTo<double>();
            Assert.AreEqual(2189.430863, value, tolerance);

            value = workbook.Evaluate(true, @"=VARP(Data!H:H)").CastTo<double>();
            Assert.AreEqual(2189.430863, value, tolerance);
        }
